...
def start_demo(cfg):
    ...
    max_img_size = 768
    num_clips = 3
    image_resize = ImageResize(max_img_size, 'bilinear')
    image_pad = ImagePad(max_img_size, max_img_size)
    image_norm = ImageNorm(
        mean=cfg.img_pixel_mean,
        std=cfg.img_pixel_std,
    )

    video_path = osp.abspath(cfg.video_path)
    vid = read_video(video_path)[0]
    T, H, W, C = vid.shape
    step = T // num_clips
    vid = vid[0:-1:step]
    T = vid.shape[0]
    ...
    vid = image_resize(vid)
    vid = image_pad(vid)
    vid = vid.unsqueeze(0).to(device)
    vid = image_norm(vid)

    # img_path = osp.abspath(cfg.video_path)
    ...
    # img = image_pad(img)
    # img = image_norm(img.unsqueeze(0).to(device))
    
    tokenizer = BertTokenizerFast.from_pretrained(cfg.tokenizer_dir)
    texts = [cfg.caption, cfg.foil]
    ...
    batch = dict(
        ...
        n_examples_list=torch.tensor([2]).to(device),
    )

    # TODO: run forward prop.
    output = forward_step(model, batch, cfg)
    print(output['itm_scores'])
    import ipdb; ipdb.set_trace()
...
if __name__ == '__main__':
    input_cfg = shared_configs.get_demo_args()
